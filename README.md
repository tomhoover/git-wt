...
| `remove <branch> [-f|--force] [-d|--delete-branch]` | `r`, `rm` | <ul><li>Remove the worktree for `<branch>`</li><li>`-f` removes a dirty worktree and bypasses confirmation for partial matches</li><li>`-d` also deletes the branch using `git branch -d` (refuses if unmerged — `-f` does **not** force branch deletion)</li><li>Partial matches (prefix/segment) require confirmation unless `-f` is used</li></ul> |
