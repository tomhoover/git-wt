...
| `remove [-f\|--force] [-d\|--delete-branch] <branch>` | `r`, `rm` | Remove the worktree for `<branch>`; `-f` removes a dirty worktree; `-d` also deletes the branch using `git branch -d` (refuses if unmerged — `-f` does **not** force branch deletion) |
| `prune` | `p`, `pr` | Prune stale worktree references |
| `cd [<branch>]` | `c` | Print path to worktree; accepts exact, prefix, or segment match; no arg = main worktree, or launches `fzf` selector when multiple worktrees exist (use with `cd` — see below) |
...
git wt remove -f -d feature-x  # force-remove dirty worktree and delete branch
                               # note: -f only forces the worktree removal; branch
                               # deletion always uses 'git branch -d' (safe, never -D)
